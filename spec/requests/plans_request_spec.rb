...
RSpec.describe "旅行の新規登録", type: :request do
  ...
  context "ログインしているユーザーの場合" do
    ...
    it "有効な旅行企画が登録できること" do
      expect { post plans_path, params: { plan: {  title: "みんなで行こうオーストラリアのグレートバリアリーフ",
                                                   date: "2021年4月1日",
                                                   place: "オーストラリア",
    ...
    it "無効な旅行企画は登録できない" do
      expect {
      post plans_path, params: { plan: {  title: "", date: "2021年4月1日", place: "オーストラリア", meeting_place: "オーストラリア空港", meeting_time: "13:00", content: "オーストラリアに行きましょう！"} }}.not_to change(Plan, :count)
      expect(response).to render_template('plans/new')
    end
    end
