...
RSpec.describe "コメント機能", type: :request do
  ...
  let!(:comment) { create(:comment, user_id: user.id, plan: plan) }
  ...
  context "コメントの登録" do
    context "ログインしている場合" do
      ...
      it "有効な内容のコメントが登録できること" do
        expect {
          post comments_path, params: { plan_id: plan.id,
                                        comment: { content: "いいですね！" } }
        }.to change(plan.comments, :count).by(1)
      end
      ...
      it "無効な内容のコメントが登録できないこと" do
        expect {
          post comments_path, params: { plan_id: plan.id,
                                        comment: { content: "" } }
        }.not_to change(plan.comments, :count)
      end
    end
